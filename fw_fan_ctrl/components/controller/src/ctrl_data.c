...
static inline void param_set_value(ctrl_param_t params[], uint8_t ch, float val) {
    if (ch >= CTRL_NUM_CHANNELS) return;
...
static inline float param_get_value(const ctrl_param_t params[], uint8_t ch) {
    if (ch >= CTRL_NUM_CHANNELS) return -1.0f;
...
void ctrl_set_setpoint_value(uint8_t ch, float v) { param_set_value(set_point, ch, v); }
void ctrl_set_setpoint_min(uint8_t ch, float m)   { param_set_min(set_point, ch, m); }
void ctrl_set_setpoint_max(uint8_t ch, float m)   { param_set_max(set_point, ch, m); }
...
void ctrl_set_duty_value(uint8_t ch, float v) { param_set_value(duty, ch, v); }
void ctrl_set_duty_min(uint8_t ch, float m)   { param_set_min(duty, ch, m); }
void ctrl_set_duty_max(uint8_t ch, float m)   { param_set_max(duty, ch, m); }
...
void ctrl_set_kp_value(uint8_t ch, float v) { param_set_value(kp, ch, v); }
void ctrl_set_kp_min(uint8_t ch, float m)   { param_set_min(kp, ch, m); }
void ctrl_set_kp_max(uint8_t ch, float m)   { param_set_max(kp, ch, m); }
...
void ctrl_set_ki_value(uint8_t ch, float v) { param_set_value(ki, ch, v); }
void ctrl_set_ki_min(uint8_t ch, float m)   { param_set_min(ki, ch, m); }
void ctrl_set_ki_max(uint8_t ch, float m)   { param_set_max(ki, ch, m); }
...
void ctrl_set_kd_value(uint8_t ch, float v) { param_set_value(kd, ch, v); }
void ctrl_set_kd_min(uint8_t ch, float m)   { param_set_min(kd, ch, m); }
void ctrl_set_kd_max(uint8_t ch, float m)   { param_set_max(kd, ch, m); }
...
bool ctrl_get_output_state(uint8_t ch) { 
    if (ch >= CTRL_NUM_CHANNELS) return false;
...
ctrl_type_t ctrl_get_controller_type(uint8_t ch) { 
    if (ch >= CTRL_NUM_CHANNELS) return CTRL_OPEN;
